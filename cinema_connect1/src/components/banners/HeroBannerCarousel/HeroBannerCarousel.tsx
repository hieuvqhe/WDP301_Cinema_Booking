...
export const HeroBannerCarousel = ({ 
  ...
  useEffect(() => {
    if (isAutoPlaying && banners && banners.length > 1) {      intervalRef.current = setInterval(() => {
  ...
  if (isLoading) {
    ...
    console.log('No banners available, showing fallback. Banners:', banners);
    return (
      <section className="relative h-[70vh] bg-gradient-to-r from-violet-900 via-purple-900 to-indigo-900 flex items-center justify-center">
        <div className="text-center text-white">
          <h2 className="text-4xl font-bold mb-4">Chào mừng đến với Cinema Connect</h2>
          <p className="text-xl text-gray-300">Trải nghiệm điện ảnh tuyệt vời</p>
          {isLoading && <p className="text-sm text-gray-400 mt-4">Đang tải banner...</p>}
        </div>
      </section>
    );
  }

  console.log('Rendering carousel with banners:', banners);

  return (
    <section className="relative h-[70vh] overflow-hidden bg-black">
      {/* Banner Slides */}
      <div className="relative h-full">        {banners.map((banner, index) => (
          <div
            key={banner._id}
                  ...
                  {banner.type && (
                    <div className="inline-block mb-4">
                      <span className="bg-orange-500 text-white px-3 py-1 rounded-full text-sm font-medium uppercase tracking-wide">
                        {banner.type === 'home_slider' ? 'Nổi bật' : 
                         banner.type === 'promotion' ? 'Khuyến mãi' : 'Thông báo'}
                      </span>
                    </div>
                  )}                  {/* Title */}
                  ...
                  <div className="flex flex-col sm:flex-row gap-3 md:gap-4">
                    {banner.link_url && (
                      <Button
                        ...
                        size="lg"
                      >
                        <Play className="mr-2 transition-transform duration-300 group-hover:scale-110 group-hover:rotate-6" size={18} />
                        Đặt vé ngay
                      </Button>
                    )}
                    ...
                    <Button
                      ...
                      size="lg"
                    >
                      <ExternalLink className="mr-2 transition-transform duration-300 group-hover:scale-110 group-hover:rotate-12" size={18} />
                      Tìm hiểu thêm
                    </Button>
                  </div>
                </div>
